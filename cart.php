    ...
    <header class="header">
        ...
        <div class="row">
            <table class="table table-bordered">
                        ...
                        foreach($_SESSION['cart'] as $item){
                            ...
                            echo'<tr> 
                                    ...
                                    <td style="display:flex;">
                                        <button class="btn btn-light" style="border-radius: 0px;border: 1px solid #e5e5e5;" onclick="addMoreCart('.$item['id'].',-1)">-</button>
                                        <input type="number" name="num" id="num_'.$item['id'].'" step="1" class="form-control" value="'.$item['num'].'" style="border-radius:0px;width:100px;" onchange="fixCartNum('.$item['id'].')">
                                        <button class="btn btn-light" style="border-radius: 0px;border: 1px solid #e5e5e5;" onclick="addMoreCart('.$item['id'].',1)">+</button>
                                    </td>
                                    <td>'.number_format($item['discount'] * $item['num']).'VND</td>
                                    <td><button class="btn btn-dark" style="border-radius:0px" onclick="updateCart('.$item['id'].',0)">Delete</button></td>
                                </tr>';
                        }
                ?>    
            </table>
                ...
                <div class="pay">
                    <h2>ORDER SUMMARY</h2>
                    <span>Shipping fee is not included</span>
                    <h5><b>Total money :</b> <?=number_format($total)?>VND</h5>
                    <a href="../Assignment-PHP/checkout.php"><button class="btn btn-dark" style="border-radius:0px;width: 404px;margin-bottom: 10px;">CONTINUE PAYING</button></a> <br>
                    <a href="../Assignment-PHP/index.php"><button class="btn btn-light" style="border-radius:0px;width: 404px;border: 1px solid;border-bottom: 2px solid #000">CONTINUE SHOPPING</button></a>
                </div>
        </main>
                    </div>
                </div>
